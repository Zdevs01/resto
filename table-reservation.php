...
">RESERVATION</h1>
    ...
    <div class="container">
        ...
        if(isset($_POST['submit_table_reservation_form']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                $stmtgetCurrentClientID = $con->prepare("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'resto' AND TABLE_NAME = 'clients'");
                ...
                $stmt_reservation->execute([date("Y-m-d H:i"), $client_id[0], $desired_date, $number_of_guests, $table_id]);
                
                $reservation_id = $con->lastInsertId();
                
                echo "<div class='alert alert-success'>✨ Super ! Votre réservation a été créée avec succès. 🏷️</div>";
                echo "<a href='generate_invoice.php?reservation_id=$reservation_id' class='btn btn-primary'>Télécharger la facture</a>";
...
<form method="POST" action="table-reservation.php">
    <div class="row">
        ...
        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-6">
            <div class="form-group">
                <label for="number_of_guests">Combien de personnes ?</label>
                <select class="form-control" name="number_of_guests">
                    <option value="1" <?php echo (isset($_POST['number_of_guests']))?"selected":"";  ?>>
                        Une personne
                    </option>
                    <option value="2" <?php echo (isset($_POST['number_of_guests']))?"selected":"";  ?>>Deux personnes</option>
                    <option value="3" <?php echo (isset($_POST['number_of_guests']))?"selected":"";  ?>>Trois personnes</option>
                    <option value="4" <?php echo (isset($_POST['number_of_guests']))?"selected":"";  ?>>Quatre personnes</option>
                </select>
            </div>
        </div>
        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-6">
            <div class="form-group">
                <label for="check_availability" style="visibility: hidden;">Vérifier la disponibilité</label>
                <input type="submit" class="form-control check_availability_submit" name="check_availability_submit" value="Vérifier la disponibilité">
            </div>
        </div>
    </div>
</form>
            ...
            <?php
                ...
                if(isset($_POST['check_availability_submit']))
                {
                    ...
                    if ($stmt->rowCount() == 0) {
                        ?>
                        <div class="error_div">
                            <span class="error_message" style="font-size: 16px">TOUTES LES TABLES SONT RÉSERVÉES</span>
                        </div>
                        <?php
                    } else {
                        ...
                        <form method="POST" action="table-reservation.php">
                            ...
                            <div class="client_details_tab">
                                <div class="form-group colum-row row">
                                    <div class="col-sm-12">
                                        <input type="text" name="client_full_name" id="client_full_name" oninput="document.getElementById('required_fname').style.display = 'none'" onkeyup="this.value=this.value.replace(/[^\sa-zA-Z]/g,'');" class="form-control" placeholder="Nom complet">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-6">
                                        <input type="text" name="client_phone_number" id="client_phone_number" oninput="document.getElementById('required_phone').style.display = 'none'" class="form-control" onkeyup="this.value=this.value.replace(/[^0-9]/g,'');" placeholder="Numéro de téléphone">
                            ...
                            <div class="form-group">
                                <input type="submit" name="submit_table_reservation_form" class="btn btn-info" value="Réserver une table">
                            </div>
                        </form>
                        <?php
                    }
